  ...
  try {
    ...
    try {
      ...
      return {
        ...
        headers: {
          ...SECURITY_HEADERS,
          "Content-Type": contentType.includes("application/json") ? contentType : SECURITY_HEADERS["Content-Type"]
        },
        body: contentType.includes("application/json") ? body : JSON.stringify({ error: "Upstream did not return JSON" })
      };
    } finally {
    ...
    return jsonResponse(statusCode, {
      error: error.name === "AbortError" ? "Pokemon TCG API request timed out" : error.message
    });
  }
};
